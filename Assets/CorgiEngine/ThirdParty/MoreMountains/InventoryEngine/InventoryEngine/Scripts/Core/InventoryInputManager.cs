...
namespace MoreMountains.InventoryEngine
{
	...
	public class InventoryInputManager : MonoBehaviour, MMEventListener<MMInventoryEvent>
	{
		...
		protected virtual void HandleInventoryInput()
		{
			...
			#if ENABLE_INPUT_SYSTEM
				_toggleInventoryKeyPressed = WasPressedThisFrameSafe(ToggleInventoryKey);
				_cancelKeyPressed = WasPressedThisFrameSafe(CancelKey);
				_prevInvKeyPressed = WasPressedThisFrameSafe(PrevInvKey);
				_nextInvKeyPressed = WasPressedThisFrameSafe(NextInvKey);
				_moveKeyPressed = WasPressedThisFrameSafe(MoveKey);
				_equipOrUseKeyPressed = WasPressedThisFrameSafe(EquipOrUseKey);
				_equipKeyPressed = WasPressedThisFrameSafe(EquipKey);
				_useKeyPressed = WasPressedThisFrameSafe(UseKey);
				_dropKeyPressed = WasPressedThisFrameSafe(DropKey);
			#else
		...
		#if ENABLE_INPUT_SYSTEM
		/// <summary>
		/// Safely reads a key from the Input System. Some older prefabs serialize legacy key
		/// values that are invalid for Unity's newer Keyboard indexer and throw every frame.
		/// </summary>
		protected virtual bool WasPressedThisFrameSafe(Key key)
		{
			...
			if (!Enum.IsDefined(typeof(Key), key) || ((int)key <= 0))
			{
				return false;
			}
