...
{	
	...
	public class DamageOnTouch : MonoBehaviour 
	{
		...
		protected Character _colliderCharacter;
		...
		protected virtual void Colliding(Collider2D collider)
		{
			...
			{
				_colliderHealth = collider.gameObject.GetComponentInParent<Health>();
			}

			if (IsOwnerHealth(_colliderHealth))
			{
				return;
			}

			_colliderCharacter = (_colliderHealth != null)
				? _colliderHealth.gameObject.MMGetComponentNoAlloc<Character>()
				: collider.gameObject.GetComponentInParent<Character>();
			bool colliderLayerMatches = MMLayers.LayerInLayerMask(collider.gameObject.layer, TargetLayerMask);
			bool healthLayerMatches = (_colliderHealth != null) && MMLayers.LayerInLayerMask(_colliderHealth.gameObject.layer, TargetLayerMask);
			bool targetIsPlayerCharacter = (_colliderCharacter != null) && (_colliderCharacter.CharacterType == Character.CharacterTypes.Player);

			// if what we're colliding with isn't part of the target layers, we do nothing and exit
			if (!colliderLayerMatches && !healthLayerMatches && !targetIsPlayerCharacter)
			{
				return;
			}
		...
		protected virtual bool IsOwnerHealth(Health collidingHealth)
		{
			if ((Owner == null) || (collidingHealth == null))
			{
				return false;
			}

			return (collidingHealth.gameObject == Owner) || collidingHealth.transform.IsChildOf(Owner.transform);
		}
