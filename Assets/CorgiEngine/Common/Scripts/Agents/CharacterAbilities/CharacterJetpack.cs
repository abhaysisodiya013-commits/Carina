...
{	
	...
	public class CharacterJetpack : CharacterAbility 
	{
		...
		[Header("Feel")]
		/// whether or not the jetpack should ramp towards its force instead of instantly snapping to it
		[Tooltip("whether or not the jetpack should ramp towards its force instead of instantly snapping to it")]
		public bool SmoothJetpackForce = false;
		/// how fast the jetpack reaches its target force when SmoothJetpackForce is true
		[Tooltip("how fast the jetpack reaches its target force when SmoothJetpackForce is true")]
		public float JetpackForceAcceleration = 20f;
		...
		protected bool _jetpacking = false;
		...
		public virtual void JetpackStart()
		{
			if ((!AbilityAuthorized) // if the ability is not permitted
			    || (!HasEnoughFuelToStartOrContinue()) // or if there's not enough fuel left
			...
			if (FuelLeft)
			{
				...
				float targetVerticalForce = JetpackForce;
				if (SmoothJetpackForce)
				{
					float currentVerticalForce = Mathf.Max(0f, _controller.ForcesApplied.y);
					targetVerticalForce = Mathf.MoveTowards(currentVerticalForce, JetpackForce, JetpackForceAcceleration * Time.deltaTime);
				}
				_controller.SetVerticalForce (targetVerticalForce);
			} 
		...
		/// <summary>
		/// Returns true if the jetpack has enough fuel to start or continue its current burst.
		/// </summary>
		protected virtual bool HasEnoughFuelToStartOrContinue()
		{
			...
			{
				return JetpackFuelDurationLeft > 0f;
			}

			return JetpackFuelDurationLeft >= MinimumFuelRequirement;
		}
