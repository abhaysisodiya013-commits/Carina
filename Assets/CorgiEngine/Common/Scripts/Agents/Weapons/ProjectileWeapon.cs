...
namespace MoreMountains.CorgiEngine
{
	...
	public class ProjectileWeapon : Weapon
	{
		...
		/// if true, projectiles per shot will be fired one after another instead of all on the same frame
		[Tooltip("if true, projectiles per shot will be fired one after another instead of all on the same frame")]
		public bool FireProjectilesSequentially = false;
		/// delay between each projectile when Fire Projectiles Sequentially is enabled
		[MMCondition("FireProjectilesSequentially", true)]
		[Tooltip("delay between each projectile when Fire Projectiles Sequentially is enabled")]
		public float DelayBetweenSequentialProjectiles = 0.2f;
		/// if true, a new sequential burst won't start while the previous one is still firing
		[MMCondition("FireProjectilesSequentially", true)]
		[Tooltip("if true, a new sequential burst won't start while the previous one is still firing")]
		public bool PreventOverlappingSequentialBursts = true;
		...
		protected Coroutine _sequentialProjectileCoroutine;
		...
		{
			if (FireProjectilesSequentially && PreventOverlappingSequentialBursts && (_sequentialProjectileCoroutine != null))
			{
				return;
			}
			...
			if (FireProjectilesSequentially && (ProjectilesPerShot > 1))
			{
				_sequentialProjectileCoroutine = StartCoroutine(SpawnProjectilesSequentiallyCo(SpawnPosition, ProjectilesPerShot));
				return;
			}
		...
		/// <summary>
		/// Spawns the configured projectiles over time instead of all at once.
		/// </summary>
		protected virtual IEnumerator SpawnProjectilesSequentiallyCo(Vector3 spawnPosition, int totalProjectiles)
		{
			for (int i = 0; i < totalProjectiles; i++)
			{
				SpawnProjectile(spawnPosition, i, totalProjectiles, true);

				if (i < totalProjectiles - 1)
				{
					yield return new WaitForSeconds(Mathf.Max(0f, DelayBetweenSequentialProjectiles));
				}
			}

			_sequentialProjectileCoroutine = null;
		}
