...
namespace MoreMountains.CorgiEngine
{
	...
	public class ComboWeapon : MonoBehaviour
	{
		...
		/// <summary>
		/// Forces the combo to a specific weapon index, useful when another ability wants to branch straight to a specific combo hit.
		/// </summary>
		public virtual bool ForceWeaponIndex(int weaponIndex)
		{
			if ((Weapons == null) || (Weapons.Length == 0) || (weaponIndex < 0) || (weaponIndex >= Weapons.Length))
			{
				return false;
			}

			OwnerCharacterHandleWeapon = (OwnerCharacterHandleWeapon != null) ? OwnerCharacterHandleWeapon : Weapons[_currentWeaponIndex].CharacterHandleWeapon;
