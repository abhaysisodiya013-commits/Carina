...
{	
	...
	public class Ladder : MonoBehaviour 
	{
		...
		[Header("Gizmos")]
		[Tooltip("Shows dots for the ladder center, player center target, and ladder top/end in the Scene view.")]
		public bool ShowLadderCenterGizmos = true;
		public Color LadderCenterGizmoColor = Color.cyan;
		public Color PlayerCenterGizmoColor = Color.yellow;
		public Color LadderTopGizmoColor = Color.green;
		public float LadderGizmoDotSize = 0.08f;
		...
		{
			if (!ShowLadderCenterGizmos)
			{
				return;
			}

			Collider2D ladderCollider = GetComponent<Collider2D>();
			Vector3 center = (ladderCollider != null) ? ladderCollider.bounds.center : transform.position;
			Vector3 top = center;
			if (ladderCollider != null)
			{
				top.y = ladderCollider.bounds.max.y;
			}

			Gizmos.color = LadderCenterGizmoColor;
			Gizmos.DrawSphere(center, LadderGizmoDotSize);
			Gizmos.color = LadderTopGizmoColor;
			Gizmos.DrawSphere(top, LadderGizmoDotSize);
